...
const QUESTION_RESPONSE_FORMAT = {
  ...
  schema: {
    ...
    required: ['content', 'exampleAnswer', 'tags'],
  },
} as const;
    ...
    input: [
      {
        ...
        content: [
          ...
          '',
          '규칙:',
          '- content와 exampleAnswer는 모두 한국어 존댓말로 작성합니다.',
          '- content는 하나의 질문 문장으로 작성합니다.',
          '- content에 지원자, 후보자, 사용자, 당신 같은 답변자 지칭 표현을 사용하지 마세요.',
          '- 질문은 직무, 경력, 기술 스택 중 하나 이상과 직접 관련되어야 합니다.',
          '- "이번 질문의 중심 기술"이 제공되면 질문과 예시 답변은 해당 기술을 중심으로 작성합니다.',
          '- "참고 가능한 보조 기술"은 중심 기술을 설명하거나 실무 맥락을 만들 때만 최대 1개까지 사용합니다.',
          '- 질문과 예시 답변이 여러 기술을 병렬로 설명하거나 여러 주제로 확장되지 않게 합니다.',
          '- 너무 광범위한 개념 설명 질문보다 실제 면접에서 답변을 평가할 수 있는 구체적인 질문을 만드세요.',
          '- exampleAnswer는 content 질문에 직접 답하는 좋은 예시 답변으로 작성합니다.',
          '- exampleAnswer는 핵심 개념, 판단 기준, 실무적 근거와 피드백 평가에 필요한 핵심 키워드를 명확히 포함합니다.',
          '- exampleAnswer에는 예시 답변 본문만 작성하고, 평가 기준이나 키워드 목록 같은 메타 설명을 포함하지 마세요.',
          ...
          '- exampleAnswer는 500자 이내로 작성합니다.',
          '- content와 exampleAnswer에 코드/마크다운/백틱/줄바꿈을 포함하지 마세요.',
        ].join('\n'),
      },
      {
        role: 'user',
