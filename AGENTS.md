...
- 커밋 본문 bullet은 별도의 `-m`으로 나누지 않고, 하나의 메시지 블록으로 작성한다.
  - 금지: `git commit -m "type: 변경 내용 요약" -m "- 변경 내용 1" -m "- 변경 내용 2"`
  - Git은 여러 `-m` 값을 각각 별도 문단으로 처리하므로 bullet 사이에 빈 줄이 생긴다.
  - 본문이 여러 줄이면 본문 전체를 하나의 메시지 인자로 전달하거나 커밋 메시지 파일을 사용한다.
...
PR 본문 작성 시 아래 기준을 따른다.

- 테스트나 검증 결과는 기본적으로 PR 본문에 적지 않는다.
- 리뷰어가 알아야 할 수동 확인 내용이나 테스트를 하지 못한 이유가 있을 때만 `작업 상세 내용`에 포함한다.
